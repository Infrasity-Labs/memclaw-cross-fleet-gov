...
from config import FLEET_ORG_SHARED, FLEET_LEGAL, AGENT_LEGAL
...
class LegalFleet:
    ...
    def write_private(self, content, memory_type=None):
        return self.client.write_memory(FLEET_LEGAL, self.agent_id, content, memory_type)
    ...
    def recall_private(self, query, top_k=5):
        return self.client.recall([FLEET_LEGAL], self.agent_id, query, top_k)

    def recall_all(self, query, top_k=5):
        """Single governed recall across all authorized legal fleets."""
        return self.client.recall([FLEET_ORG_SHARED, FLEET_LEGAL], self.agent_id, query, top_k)
